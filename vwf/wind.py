...
def simulate_grid_speed(reanalysis, turb_info):   
    # heights are assigned to allow speed to be calculated for every height at every gridpoint
    reanalysis = reanalysis.assign_coords(
        height=('height', turb_info['height'].unique()))
...
def simulate_speed(reanalysis, turb_info, *args):
    ws = simulate_speed_grid(reanalysis, turb_info)
    ...
    if len(args) >= 1: 
        bc_factors = args[0]
        time_res = args[1]
        sim_ws = correct_speed(sim_ws, time_res, bc_factors, turb_info)
        
    if save == True:
        sim_ws.to_pandas().reset_index()
    
    return sim_ws



def simulate_cf(reanalysis, turb_info, power_curves, *args):
    sim_ws = simulate_speed(reanalysis, turb_info, *args)
    
    def speed_to_power(data):
        x = power_curves['data$speed']          
        y = power_curves[data.model[0].data]
    ...
    if save == True:
        sim_cf.to_pandas().reset_index()
        
    return sim_cf


def correct_speed(ds, time_res, bc_factors, turb_info):
    """
...
def train_simulate_cf(reanalysis, turb_info, power_curves, scalar=1, offset=0):
    """
    ...
    """ 
    unc_ws = simulate_speed(reanalysis, turb_info)
