...
def get_time_slice_name(time_res: str, month: int = None) -> str:
    """Get the time slice name for a given resolution and month.

    Args:
        time_res: Temporal resolution ('fixed', 'season', 'bimonth', 'month').
        month: Month number (1-12). Required for non-fixed resolutions.

    Returns:
        Time slice string (e.g., 'winter', '1/6', '1/1', '3').

    Raises:
        ValueError: If month is required but not provided.

    Examples:
        >>> get_time_slice_name('fixed')
        '1/1'
        >>> get_time_slice_name('season', month=1)
        'winter'
        >>> get_time_slice_name('bimonth', month=7)
        '4/6'
        >>> get_time_slice_name('month', month=3)
        '3'
    """
    if time_res == 'fixed':
        return '1/1'
    ...
    if month is None:
        raise ValueError(f"month parameter required for time_res='{time_res}'")

    if time_res == 'month':
        return str(month)

    # Create temporary DataFrame to use add_time_resolution_columns
    temp_df = pd.DataFrame({'month': [month]})
    temp_df = add_time_resolution_columns(temp_df)

    return temp_df[time_res].iloc[0]
