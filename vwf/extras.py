...
def calc_metrics_era5(cluster_list, year_test, country):
    ...
    cf_obs = obs = pd.read_csv('data/results/raw/'+country+"_"+str(year_test)+'_obs_cf.csv', parse_dates=['time'])
    ...
    cf_uncorr = pd.read_csv('data/results/raw/'+country+"_"+str(year_test)+'_unc_cf.csv', parse_dates=['time'])
    ...
    for num_clu in cluster_list:
        for time_res in time_res_list:
            ...
            cf_corr = pd.read_csv('data/results/raw/'+country+"_"+str(year_test)+'_'+time_res+'_'+str(num_clu)+'_cor_cf.csv', parse_dates=['time'])
    ...
    clus_metrics.to_csv('data/results/'+country+"_"+str(year_test)+'_metrics.csv', index = None)
