...
Phase 1 design written (`docs/design/HARNESS_DESIGN.md`), awaiting review. No
...
### Design decisions proposed (see docs/design/HARNESS_DESIGN.md)
...
**Status:** `docs/design/HARNESS_DESIGN.md` rev 2 written, awaiting confirmation
...
- **R3 (DK only): Energy-paper numbers.** Needs the user to point at the
...
- NL (proposed country-level choice): clusters {5} × {fixed}, train
  ...
  vs mean_obs 0.143, docs/findings/TURBINE_GRID_EVALUATION_ANALYSIS.md). The
...
- NL reproduces the DOCUMENTED failure mode
  (docs/findings/TURBINE_GRID_EVALUATION_ANALYSIS.md): uncorrected over-predicts
...
- **Cross-check against an independent AU CF reference** if one exists
  (published NEM capacity factors / Renewables.Ninja AU): an external anchor,
...
- Recent month resolves: MMSDM_2026_06 listed, updated 14 Jul 2026. The HTTPS
...
all safe; references regenerable by design, which was the method's point).
...
script and AEMO fetch script WRITTEN and shown, NOT committed, NOT run: the
...
- Partition-invariance framing adopted: pillar A's JJA gate is THE critical
  check, not one of four, and stays front and centre at pillar A.
...
- vwf/datasets/aemo_au.py: MMS C/I/D/F parser (validated against a real
  ...
  unit-group rows, e.g. Boco Rock 9x1.60+58x1.70; "In Service/In Commissioning"
...
NOTE: Bango 973/999 are two DUIDs under one GWPT project, so many-to-one
...
- If approved: 97/105 DUIDs (~92%), from 61%. Requires a phase-aware alias
  ...
  capacity-weighted centroid); implementation follows per-line approval.
...
- CROOKWF2: GI 28 × 3.43 vs Wikipedia Crookwell 2 = 46 units/92 MW;
  conflicting, so kept FALLBACK rather than forced.
...
Vestas machines are V27/V29/V47/V82 (225 kW–1.65 MW), so its "manufacturer
...
It would upgrade the public repo from "runs on invented curves" to "runs on
...
data; pre-commit checks passed with zero baked outputs, no local paths,
...
- AU→DK: the capacity-weighted collapse of AU's OPPOSING clusters lands
  near identity (scalars 0.98–1.10), i.e. cancellation; transfer applies a
...
ask about absolute skill, so this likely warrants an amendment. USER TO RULE.
...
- Net generation can be genuinely negative in a calm month (parasitic load >
  output). Kept, not clipped (the same non-clipping choice as AU), so the
...
- **`docs/runbooks/US.md`**: the full reproduction path (download → process →
...
- **Fleet ≈ 133 GW** nameplate, matching the real 2021 US wind fleet.
...
- **Wiring**: registered; BR bbox (whole country, crosses the equator);
  ...
  `scripts/fetch_era5_br.py` + `combine_era5_br_daily.py`; `docs/runbooks/BR.md`.
...
- 152 wind complexes with coordinates, ≈ 26 GW, matching the real ONS fleet.
