...
prepare_ninja_files = function(files, processNewestFirst=FALSE)
{
	merra = list()

	# check our NetCDF files exist
	merra$files = files
	if (!all(file.exists(files)))
		stop("prepare_ninja_files: not all of your NetCDF files exist!\n\n", files)

	# run in reverse order if desired
	if (processNewestFirst == TRUE)
		merra$files = rev(merra$files)

	# extract their simple names
	merra$names = get_text_before(basename(merra$files), '.nc')

	# extract the dates from filenames
	reggy = regexpr("(\\d{4}.?\\d?\\d?.?\\d?\\d?)", merra$names)
	merra_dates = regmatches(merra$names, reggy)

	# pad yearly / monthly files to full date
	md_yy = nchar(merra_dates)==4
	md_ym = nchar(merra_dates)==7
	merra_dates[ md_yy ] = merra_dates[ md_yy ] %&% '-01-01'
	merra_dates[ md_ym ] = merra_dates[ md_ym ] %&% '-01'
	merra$dates = ymd(merra_dates)
...
NetCdfClass = function(filename, model, printInfo=FALSE)
{
	...
	NetCdf$constructor = function(filename, model, printInfo=FALSE)
	{
		...
		if (NetCdf$model == 'ERA5')
		{
			NetCdf$lon = ncvar_get(NetCdf$ncdf, 'lon')
			NetCdf$lat = ncvar_get(NetCdf$ncdf, 'lat')
		}

		if (NetCdf$model == 'ERA5NEW')
		{
			NetCdf$lon = ncvar_get(NetCdf$ncdf, 'longitude')
			NetCdf$lat = ncvar_get(NetCdf$ncdf, 'latitude')
		}
		...
		if (min(NetCdf$lon)>=0) NetCdf$lon = NetCdf$lon - 180
	...
	NetCdf$open_file = function(filename, printInfo=FALSE)
	{
		...
		if (NetCdf$model %in% c('ERA5', 'ERA5NEW'))
		{
			library(ncdf4.helpers)
			nc_time = nc.get.time.series(nc)
			nc_time = as.POSIXct(nc_time, origin="1970-01-01")

			NetCdf$date = date(nc_time)
			NetCdf$hour = as.numeric(time(nc_time))
		}
...
#### THAT FUCKING BULLSHIT CLASS BOLLOCKS IS SUPER DEPRECATED AND JUST PLAIN GARBAGE
#### NOW WE GET INTO MEGA HACKING AND START USING BITS OF THE VWF.NETCDF.R LIBRARY HERE
#### HAHAHA POOR FUCKING FOLKS TRYING TO USE THIS BAWBAG CODE
