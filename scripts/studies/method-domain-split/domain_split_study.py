...
CONFIGURATIONS = (
    ("BE", "all", "country", 2023, 3, 0.0192), ("DE", "onshore", "turbine", 2019, 500, 0.0411),
    ("DK", "offshore", "turbine", 2020, 2, 0.1113), ("DK", "onshore", "turbine", 2020, 700, 0.0678),
    ("ES", "all", "country", 2023, 4, 0.0255), ("FR", "all", "country", 2023, 10, 0.0301),
    ("IE", "all", "country", 2023, 3, 0.1563), ("IT", "all", "country", 2023, 3, 0.0308),
    ("NL", "all", "country", 2023, 5, 0.0563), ("NO", "all", "country", 2023, 5, 0.0381),
...
POOL_CODE = {("DK", "offshore"): "DK-offshore", ("DK", "onshore"): "DK-onshore",
             ("UK", "offshore"): "UK-offshore", ("UK", "onshore"): "UK-onshore",
...
def self_weight(pool: pd.DataFrame, code: str, mode: str) -> float:
    ...
    cells = np.unique(np.column_stack([
        GRID_LON[np.abs(mine["lon"].to_numpy()[:, None] - GRID_LON[None, :]).argmin(axis=1)],
        GRID_LAT[np.abs(mine["lat"].to_numpy()[:, None] - GRID_LAT[None, :]).argmin(axis=1)]]),
        axis=0)
...
def distance_masked(scalar, offset, points: pd.DataFrame) -> xr.Dataset:
    ...
    far = interp.distance_to_nearest(points, lon_grid.ravel(), lat_grid.ravel(),
                                     metric="degrees").reshape(lat_grid.shape)
...
def kriged(points: pd.DataFrame):
    return interp.to_grid(interp.kriging_at, points, GRID_LON, GRID_LAT,
...
def build_surfaces(pool: pd.DataFrame, shapes: Path = SHAPES) -> dict[str, xr.Dataset]:
    ...
    s2 = xr.Dataset(
        ...
        coords={"lat": GRID_LAT, "lon": GRID_LON})
    s1 = distance_masked(np.where(np.isnan(split_scalar), 1.0, split_scalar),
                         np.where(np.isnan(split_offset), 0.0, split_offset), pool)
...
def main(out_dir: str, pool_path: Path = POOL, runs: Path = RUNS, shapes: Path = SHAPES,
    ...
    for code, mode, level, year, clusters, published in CONFIGURATIONS:
        ...
        rows.append({**common, "condition": "uncorrected",
                     **evaluate.skill(unc_cf, observed, fleet, level),
                     "off_curve_share": unc_off["off_curve_share"]})
        rows.append({**common, "condition": f"cluster fixed_{clusters}",
        ...
        for name, field in surfaces.items():
            ...
            rows.append({**common, "condition": name, **got, **summary,
                         "off_curve_share": off["off_curve_share"]})
            print(f"  {name}: neutral {summary['neutral_share']:.1%}, "
