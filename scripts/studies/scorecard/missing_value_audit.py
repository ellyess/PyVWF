    ...
    for path in sorted(ev.glob("cor_cf_*.csv")):
        ...
        if is_country:
            ...
            clus_info = cluster_turbines(
                int(num_clu), train_fleet, False, turb_info, min_cluster_size=spec.min_cluster_size
            )
        ...
        same_val = np.allclose(
            cor_cf.to_numpy(), saved.to_numpy(), equal_nan=True, rtol=0, atol=1e-12
        )
        ...
        slices = add_time_resolution_columns(
            pd.DataFrame(
                {"time": saved.index, "year": saved.index.year, "month": saved.index.month}
            ),
            spec.seasons,
        )[time_res].to_numpy()
        ...
        capacity = clus_info.assign(ID=clus_info["ID"].astype(str)).set_index("ID")["capacity"][
            units
        ]
        ...
        units_hit = {
            r: [u for j, u in enumerate(units) if (route[:, j] == r).any()] for r in ROUTES
        }
        ...
        summaries.append(
            {
                ...
                **{
                ...
                if partly.to_numpy().any()
                else 0.0,
                "max_scalar": quality["max_scalar"],
                "n_implausible_scalar": quality["n_implausible_scalar"],
            ...
            by_cluster.append(
                {
                    ...
                    "scalar_min": float(f["scalar"].min()),
                    "scalar_max": float(f["scalar"].max()),
                    "offset_min": float(f["offset"].min()),
                    "offset_max": float(f["offset"].max()),
        ...
        print(
            f"{code} {label}: {missing.mean():.2%} of unit-steps missing "
