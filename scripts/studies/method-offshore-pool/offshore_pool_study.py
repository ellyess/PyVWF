...
ROWS = (("DK", "offshore", 2020, 2), ("DK", "onshore", 2020, 700),
        ("UK", "offshore", 2019, 10), ("UK", "onshore", 2019, 300))

#: What the chapter published for these rows, for the cross-pipeline check.
PUBLISHED_UNCORRECTED = {"DK offshore": 0.0822, "DK onshore": 0.129,
...
def main(out_dir: str, pool_path: Path = POOL, runs: Path = RUNS, shapes: Path = SHAPES) -> None:
    ...
    for name, frame in conditions.items():
        ...
        print(f"{name}: onshore {counts.get('onshore', 0)}, "
              f"offshore {counts.get('offshore', 0)}")
    ...
    for code, mode, year, clusters in ROWS:
        ...
        neutral = pd.DataFrame({"ID": fleet["ID"].astype(str), "scalar": 1.0,
        ...
        rows.append({**base, "condition": "uncorrected today", **unc,
