...
the Global Wind Power Tracker (CC BY 4.0).
    ...
    PYVWF_INPUT=<stage> python scripts/studies/method-generalisation/export_au_grid_netcdf.py \\
...
def main() -> None:
    ...
    out = xr.Dataset(
        {
            ...
            "wnd100m_corrected": cor.assign_attrs(
                units="m s-1", long_name="PyVWF seasonally corrected 100 m wind speed, daily mean"
            ),
            ...
            "cluster": xr.DataArray(
                cluster_2d.astype("int8"),
                coords={"lat": ds.lat, "lon": ds.lon},
                dims=("lat", "lon"),
            ).assign_attrs(long_name="trained correction cluster applied (nearest centroid)"),
            "km_to_centroid": xr.DataArray(
                km_2d, coords={"lat": ds.lat, "lon": ds.lon}, dims=("lat", "lon")
            ).assign_attrs(units="km", long_name="distance to the applied cluster's centroid"),
        },
        attrs={
            ...
            "robustness analysis in docs/findings/region-au-nem.md, "
            ...
            "history": "scripts/studies/method-generalisation/export_au_grid_netcdf.py",
        },
    )
    ...
    print(
        f"written: {out_path} ({out_path.stat().st_size / 1e6:.0f} MB), "
