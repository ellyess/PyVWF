#!/usr/bin/env python
"""Fetch the ERA5 subset for Argentina from the Copernicus CDS.
...
running it is yours to do, like every other download on this project:
    ...
    python scripts/fetch_era5_ar.py            # 48 months, 2021-2024
    python scripts/fetch_era5_ar.py --dry-run  # list requests, submit nothing
...
request exceeds the per-request item cap and gets rejected):
    ...
    area      N -27 / W -71 / S -49 / E -56   (see bbox rationale below)
    grid      0.25 x 0.25
    format    netcdf, unarchived

**Bounding box.** The Global Wind Power Tracker puts Argentina's operating
wind fleet between latitude -47.3 and -28.7 and longitude -69.8 to -57.7
(79 farms, ~5 GW). The box adds ~1.5 degrees of margin on every side
(N -27 / W -71 / S -49 / E -56) so no farm sits on the edge. It spans the two
regimes the CAMMESA fleet actually occupies: the Buenos Aires / Pampas coast
(Cfa) and Patagonia — Chubut and Santa Cruz — in the cold-steppe westerlies
(BSk/BWk), which is the climate nothing else in the validation set covers.
If a plant is later found outside this box in the observations, screen it out
explicitly rather than let it match the nearest in-box grid cell (the US
Hawaii lesson).

At 22 x 15 degrees (88 x 60 cells) the monthly files are modest; no daily
pre-combine step is required for a box this size (unlike BR/US).

Output layout matches what the harness expects for the AR region config
(era5_path = "era5/AR"): <input-root>/era5/AR/era5_ar_<YYYY>_<MM>.nc, where
...
Window is 2021-2024 (train 2021-2023, test 2024), matching Brazil and Chile:
it holds fleet growth to ~20% (56 -> 68 plants) instead of the 14x that the
full 2011-2026 CAMMESA history would carry into the correction.
...
#: North, West, South, East — the planned ar.toml bounding box, CDS order.
AREA = [-27, -71, -49, -56]
GRID = [0.25, 0.25]
...
def main() -> None:
    ...
    out_dir = output_dir()
    plan = [
        (year, month, out_dir / f"era5_ar_{year}_{month:02d}.nc")
