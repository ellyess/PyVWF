#!/usr/bin/env python
"""Fetch the ERA5 subset for the Brazil from the Copernicus CDS.
...
running it is yours to do, like every other download on this project:
    ...
    python scripts/fetch_era5_br.py            # all 48 months, 2021-2024
    python scripts/fetch_era5_br.py --dry-run  # list requests, submit nothing
    python scripts/fetch_era5_br.py --years 2021 2022
...
request exceeds the per-request item cap and gets rejected):
    ...
    area      N 6 / W -74 / S -34 / E -34   (configs/regions/br.toml, whole country)
    grid      0.25 x 0.25
    format    netcdf, unarchived

The Brazil box spans the whole country and crosses the equator, so each
monthly file is large and a multi-year hourly load will not fit in memory —
run scripts/combine_era5_br_daily.py afterwards to reduce each year to a
daily file, exactly as the AU/US workflow does.

Output layout matches what the harness expects for the BR region config
(era5_path = "era5/BR"): <input-root>/era5/BR/era5_br_<YYYY>_<MM>.nc, where
...
#: North, West, South, East — the br.toml bounding box in CDS order (all Brazil).
AREA = [6, -74, -34, -34]
GRID = [0.25, 0.25]
...
def main() -> None:
    ...
    out_dir = output_dir()
    plan = [
        (year, month, out_dir / f"era5_br_{year}_{month:02d}.nc")
