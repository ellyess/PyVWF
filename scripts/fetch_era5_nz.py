#!/usr/bin/env python
"""Fetch the ERA5 subset for New Zealand from the Copernicus CDS.
...
running it is yours to do, like every other download on this project:
    ...
    python scripts/fetch_era5_nz.py            # all 72 months, 2019-2024
    python scripts/fetch_era5_nz.py --dry-run  # list requests, submit nothing
    python scripts/fetch_era5_nz.py --years 2023 2024
...
request exceeds the per-request item cap and gets rejected):
    ...
    area      N -36 / W 166 / S -48 / E 179   (configs/regions/nz.toml)
    grid      0.25 x 0.25
    format    netcdf, unarchived

The box covers both main islands and the whole EMI-dispatched wind fleet
(northernmost farm Te Uku ~ -37.9, southernmost Flat Hill/Kaiwera Downs
~ -46.5). It stays west of the antimeridian, so no longitude wrap handling is
needed. At 53 x 49 cells per hour the monthly files are small; no daily
pre-combine step is required for a box this size (unlike BR/US).

Output layout matches what the harness expects for the NZ region config
(era5_path = "era5/NZ"): <input-root>/era5/NZ/era5_nz_<YYYY>_<MM>.nc, where
...
#: North, West, South, East — the nz.toml bounding box in CDS order.
AREA = [-36, 166, -48, 179]
GRID = [0.25, 0.25]
...
def main() -> None:
    ...
    ap.add_argument("--years", type=int, nargs="+", default=list(YEARS),
                    help="Years to fetch (default: 2019-2024)")
    ...
    out_dir = output_dir()
    plan = [
        (year, month, out_dir / f"era5_nz_{year}_{month:02d}.nc")
