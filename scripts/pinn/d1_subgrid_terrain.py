...
def subgrid_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for region, sub in df.groupby("region"):
        ...
        for idx, r in sub.iterrows():
            ...
            z_rng = (
                float(np.nanmax(box_land) - np.nanmin(box_land))
                if np.isfinite(box_land).any()
                else np.nan
            )
            recs.append(
                dict(
...
def main():
    ...
    print(
        df.groupby("region")[["h_excess", "cell_relief", "exposure", "scalar"]]
        .agg(["mean", "std", "max"])
        .round(2)
        .to_string()
    )
    ...
    for var in ("h_excess", "cell_relief", "exposure", "aspect_ratio"):
        ...
        print(f"  {var:14s} pearson {pr:+.3f} (p={pp:.2e})   spearman {sr:+.3f} (p={sp:.2e})")
