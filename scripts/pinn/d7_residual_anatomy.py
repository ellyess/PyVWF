...
def main():
    ...
    ap.add_argument("--profile", default="power", choices=["power", "shear-log", "log"])
    args = ap.parse_args()
    ...
    frames = []
    for r in tr:
        f = predict_frame(r, model, std, density=args.density, profile=args.profile).dropna(
            subset=["cf_sim"]
        )
        f["region"] = r.code
        meta = pd.DataFrame(
    ...
    for col, (label, edges) in SLICES.items():
        if col not in res:
            continue
        v = res[col]
        b = pd.qcut(v, 6, duplicates="drop") if edges is None else pd.cut(v, edges)
        ...
        trends.append(
            dict(slice=col, label=label, mean_resid_span=span, overall_sd=float(res.resid.std()))
        )
    ...
    for code, sub in res.groupby("region"):
        ...
        print(
            f"    {code}: amplitude {float(amp.max() - amp.min()):.4f}   "
            f"month explains {share:6.1%}"
        )

    print(f"\n{'=' * 96}\n### Offshore against onshore\n")
    print(
        res.groupby("offshore")
        .agg(
            n=("resid", "size"),
            mean=("resid", "mean"),
            rmse=("resid", lambda s: float(np.sqrt((s**2).mean()))),
