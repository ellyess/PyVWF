...
def scored(tr, te, profile, epochs, seed=0):
    m, std, hist = T.fit(
        tr, hidden=16, physics=True, profile=profile, epochs=epochs, seed=seed, verbose=False
    )
...
def main():
    ...
    print(
        f"{args.region}: {tr[0].n_units} train / {te.n_units} test units, "
    ...
    for h in LEVELS:
        ...
        print(
            f"  {h:5.0f} m   power {r['power']:.4f}   shear-log {r['shear-log']:.4f}"
    ...
    for seed in args.seeds:
        ...
        print(
            f"  seed {seed}  sd {sd:4.1f} m   power {r['power']:.4f}   "
    ...
    print(
        f"  spread across assumed levels 80-120 m:                "
        f"{lvl[lvl.profile == 'power'].rmse.max() - lvl[lvl.profile == 'power'].rmse.min():.4f}"
    )
    het = res[(res.arm == "heterogeneous") & (res.profile == "power")]
    print(
        f"  RMSE with realistic height spread (mean over seeds):  {het.rmse.mean():.4f}"
        f"  (sd {het.rmse.std():.4f})"
    )
    print(f"  cost of assuming uniformity:                          {het.rmse.mean() - base:+.4f}")
    print()
    print("  Can Brazil tell the two profile forms apart?")
    for arm in ("level", "heterogeneous"):
        sub = res[res.arm == arm]
        d = sub[sub.profile == "shear-log"].rmse.values - sub[sub.profile == "power"].rmse.values
