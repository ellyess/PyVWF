...
def train_country_level(
    country: str, config: dict, output_dir: Path, country_configs: dict, args
) -> dict:
    ...
    try:
        ...
        print(
            f"  Training obs: {len(obs_train)} timesteps ({obs_train.index.year.min()}-{obs_train.index.year.max()})"
        )
        ...
        if use_year_specific:
            ...
            model.load_country_data_with_year_specific(
                obs_train, obs_test, grid_points_dir=grid_points_dir
            )
        else:
...
def train_configuration_set(
    prefix: str, config: dict, base_dir: Path, country_configs: dict = None, args=None
) -> list:
    ...
    for country in config["countries"]:
        if config["obs_level"] == "turbine":
            ...
            if country_configs is None:
                ...
                results.append(
                    {"country": country, "status": "skipped", "error": "No country configs"}
                )
                continue
...
def main():
    ...
    # Return exit code
    total_failed = sum(
        len([r for r in results if r["status"] == "failed"]) for results in all_results.values()
    )
