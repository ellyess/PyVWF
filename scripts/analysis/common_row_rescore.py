...
def main(code, out_dir, backfill=bb.BACKFILL):
    ...
    report = report.assign(
        ...
        units_wholly_excluded=[
            ";".join(scoring[s]["units_wholly_excluded"]) for s in merged["scope"]
        ],
    )
    ...
    print(
        f"{code}: {int(changed.sum())} of {len(merged)} variant rows change; "
...
def common_rows_bootstrap(code, restricted, reported, out_dir):
    ...
    for name in ("uncorrected", reported):
        ...
        d = t["cf_sim"] - t["cf_obs"]
        per_unit[name] = (
            t.assign(w=t["capacity"], e=t["capacity"] * d**2, a=t["capacity"] * d.abs())
            .groupby("ID")[["w", "e", "a"]]
            .sum()
            .sort_index()
        )
    ...
    out = {
        ...
        "uncorrected_mbe": float(
            (restricted["uncorrected"].eval("capacity * (cf_sim - cf_obs)")).sum()
            / restricted["uncorrected"]["capacity"].sum()
        ),
        "corrected_mbe": float(
            (restricted[reported].eval("capacity * (cf_sim - cf_obs)")).sum()
            / restricted[reported]["capacity"].sum()
        ),
        "cor_sse_top1": float(shares.iloc[0]),
        "cor_sse_top5": float(shares.head(5).sum()),
    }
...
def joint(code, out_dir, eval_dirs):
    ...
    years = {
        json.loads((d / "run_manifest.json").read_text())["evaluation_year"] for d in eval_dirs
    }
    if len(years) != 1:
    ...
    for d in eval_dirs[1:]:
        ...
        same = other.columns.equals(unc.columns) and np.allclose(
            other.drop(columns="time").to_numpy(float),
            unc.drop(columns="time").to_numpy(float),
            equal_nan=True,
            rtol=0,
            atol=0,
        )
    ...
    for d in eval_dirs:
        ...
        for path in sorted(d.glob("cor_cf_*.csv")):
            ...
            per_run[d.name].append(
                {
                    "label": f"{d.name}:{time_res}_{num_clu}",
                    "extra": {},
                    "head": {
                        "run": d.name,
                        "variant": spec.correction_model,
    ...
    with warnings.catch_warnings():
        ...
        for name, variants in per_run.items():
            ...
            scored, summary = driver.score_on_common_rows(
                [unc_variant, *variants], code, scratch / name
            )
            old = pd.read_csv(Path([d for d in eval_dirs if d.name == name][0]) / "metrics.csv")
            for row in scored:
                ref = old[
                    (old["variant"] == row["variant"])
                    & (old["num_clu"] == row["num_clu"])
                    & (old["time_res"] == row["time_res"])
                ].iloc[0]
                rows.append(
                    {
                        "basis": f"within:{name}",
                        **row,
                        "run": row["run"] if row["run"] != "all" else name,
                        "rmse_old": ref["rmse"],
                        "n_units_old": ref["n_units"],
        ...
        for row in scored:
            rows.append(
                {"basis": "joint", **row, "excluded_share": summary["fleet"]["excluded_share"]}
            )
