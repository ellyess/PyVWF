#!/usr/bin/env python3
"""Download and process terrain data for PyVWF ML workflows.

This script downloads terrain features needed for ML-based bias correction
covering the European study area (North Sea region + BE, DE, DK, FR, NL, NO, UK).
...
def download_file(url, output_path, description="file"):
    ...
    Args:
        ...
        description: Friendly name used in logging.
...
def extract_archive(archive_path, extract_to=None):
    """Extract a zip or gzip archive.

    Args:
        archive_path: Path to the archive file.
        extract_to: Directory to extract to (defaults to archive parent).

    Returns:
        Path to the extracted directory or file.
    """
...
def subset_netcdf(nc_path, output_path, bounds):
    """Subset a NetCDF file to study area bounds.
    ...
    Returns:
        Output NetCDF path, or None on failure.
    """
...
def compute_terrain_derivatives(elevation, resolution_deg=0.00833):
    ...
    Args:
        ...
        resolution_deg: Grid resolution in degrees (default 0.00833 ≈ 1km at 45°N).

    Returns:
        Dataset with elevation, slope, aspect, roughness, and curvature.
    """
...
def process_eudem_tiles(eudem_dir, output_path, bounds):
    ...
    Args:
        eudem_dir: Directory containing EU-DEM tiles.
        output_path: Output NetCDF path.
        bounds: Tuple of (lat_min, lat_max, lon_min, lon_max).

    Returns:
        Output path on success, or None if processing is not completed.
    """
    if not HAS_RASTERIO:
...
def main():
    """Run the terrain data download and processing workflow."""
