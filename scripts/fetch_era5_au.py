#!/usr/bin/env python
"""Fetch the ERA5 subset for AU-NEM from the Copernicus CDS, month by month.
...
running it is yours to do, like every other download on this project:
    ...
    python scripts/fetch_era5_au.py            # all 48 months, 2020-2023
    python scripts/fetch_era5_au.py --dry-run  # list requests, submit nothing
    python scripts/fetch_era5_au.py --years 2020 2021
...
request exceeds the per-request item cap and gets rejected):
    ...
    area      N -10 / W 129 / S -44 / E 154   (configs/regions/au_nem.toml)
    grid      0.25 x 0.25
    format    netcdf, unarchived

Output layout matches what the harness expects for the AU region config
(era5_path = "era5/AU"): <input-root>/era5/AU/era5_au_<YYYY>_<MM>.nc, where
<input-root> is $PYVWF_INPUT if set, else ./input. vwf.datasets.era5 globs
*.nc in that directory and combines by coordinates; roughness is derived
from the 10m/100m shear at load time (calc_z0=True), so no preprocessing
step is needed before running the harness.
...
#: North, West, South, East — the au_nem.toml bounding box in CDS order.
AREA = [-10, 129, -44, 154]
GRID = [0.25, 0.25]
...
def main() -> None:
    ...
    out_dir = output_dir()
    plan = [
        (year, month, out_dir / f"era5_au_{year}_{month:02d}.nc")
