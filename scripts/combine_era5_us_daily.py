#!/usr/bin/env python
"""Pre-combine the monthly ERA5-US files to yearly DAILY files.

The CONUS box is ~7x the AU-NEM box, so a 4-year hourly load is tens of GB in
...
wnd100m/roughness and skips recomputation.
    ...
    python scripts/combine_era5_us_daily.py            # 2019-2022
    python scripts/combine_era5_us_daily.py --years 2022
"""
import argparse
...
def combine_year(in_dir: Path, out_dir: Path, year: int) -> Path:
    target = out_dir / f"era5_us_daily_{year}.nc"
    if target.is_file():
        print(f"{year}: exists, skipping")
        return target
    files = sorted(in_dir.glob(f"era5_us_{year}_*.nc"))
    if len(files) != 12:
...
def main() -> None:
    ...
    ap.add_argument("--in-dir", default="input/era5/US")
    ap.add_argument("--out-dir", default="input/era5/US_daily")
    ap.add_argument("--years", type=int, nargs="+", default=[2019, 2020, 2021, 2022])
    args = ap.parse_args()
