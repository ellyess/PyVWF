...
| `windstats` | turbine | ES-WS (SE-WS/FI-WS pending coords) | Per-turbine monthly CF from the CONFIDENTIAL WindStats extract; coordinates joined from open GWPT via the thewindpower name mapping (mixed licence). Built by `scripts/process/windstats.py` — see `docs/RUNBOOK_ES.md`. |
