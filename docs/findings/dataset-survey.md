# Candidate observation datasets beyond DK/DE/UK/US/BR/AU
...
(`method-ml-transfer.md`) found the binding constraint on cross-region
...
|---|---|---|---|---|---|---|---|
| **New Zealand** | EA EMI `Generation_MD` (open CSV, no registration; Azure blob) | per plant, half-hourly | **1997→** | Register has capacity/commissioning, **no coords/heights**; with only ~21 farms, hand-compile from NZWEA; coords from GWPT | SH temperate westerlies ("Roaring Forties"), **complex terrain** | ~1.3 GW, ~21 farms | Best value per unit effort. License page unverified. |
...
| ~~**Peru**~~ | COES portal. **DEMOTED**: the open endpoint is per-COMPANY (62 firms), not per-plant | ~~per plant, 30-min~~ | wind 2014→ (portal depth not pinned) | Fichas técnicas (capacity, turbine count); no coords/heights | **Hyper-arid coastal desert (BWh/BWn), steady Humboldt coastal jet**: the most distinctive single regime | ~1.1 GW, ~10 plants | Tiny fleet → low effort. |
...
- **Chile: CONFIRMED and better than surveyed.** Per-plant hourly wind runs
  ...
  and the gotcha list: `docs/runbooks/cl.md`.
...
- **Argentina: CONFIRMED, and the cheapest tier-1 region found.** One ~2 MB
  ...
  **186 monthly columns, 2011-01 → 2026-06**, GWh per central. Wind
  ...
  continuation rows, so forward-fill or plants inherit the wrong province),
...
- **Global Wind Power Tracker** (verified against the local Feb-2026 file):
  coords/capacity/status/phase only, with **no hub heights, models, or turbine
  counts**. CC-BY-4.0.
...
1b. **Argentina (CAMMESA)**: CONFIRMED as the cheapest acquisition of all (one
