...
> **These numbers are not current with the code, and four of them come from
> degenerate fits.** Read this section before quoting any figure below.
>
> **Staleness.** Every figure here was produced by runs from 2026-07-23/24 and
> recorded at commit `7d634dc`. Two commits have landed since that touch
> fitting. `569a00a` (fit robustness) is a no-op by default: `min_cluster_size`
> defaults to 1 and a test pins the partition byte-identical. `53d667b`
> (`calculate_scalar` averaged observed and simulated over different plant
> samples) is **not** a no-op, and its own note says it "changes fitted scalars
> on any fleet with incomplete reporting". Its magnitude depends on whether
> reporting is selective, which that commit states is not established: the
> error stays near 0.005 when reporting is independent of output, but reaches
> 7 to 14 percent on the scalar under selective reporting at the 43 percent US
> reporting rate. Nothing below has been re-run since. Treat the figures as
> indicative of the method, not as reproducible against current HEAD, until a
> refresh lands.
>
> **Degenerate fits.** The fit-quality guards added in `569a00a` did not exist
> when this table was written, so no row carries them. Applying `fit_quality`
> to the exact configurations reported below flags four of the nine, marked
> with a dagger in the table. A degenerate fit can still score as a corrected
> win at monthly resolution, which is precisely how these survived.
...
| United States (US) | k250 fixed | **20.54** | 6 | 0 |
...
inside the ceiling with no failed offsets.
...
- **Not current with the code, and unrefreshed.** See the note under the title.
  `53d667b` changes fitted scalars on any fleet with incomplete reporting and
  landed after every run below.
- **Four of the nine turbine-level rows rest on degenerate fits** (CL, US, AR,
  ...
  usable, and CL in particular carries a fitted wind scalar of 80.23 with one
  offset that never converged.
