#!/usr/bin/env python3
"""Train an ML model for European correction factor prediction.

This script uses high-resolution correction factors from Denmark, the UK, and
Germany to train a model that predicts correction factors across Europe based on
terrain features. The trained model can be applied to regions without detailed
correction data and exported to NetCDF for downstream workflows.

Workflow:
    1. Load correction factors from DK, UK, DE (training data)
    2. Extract terrain features at correction point locations
    3. Train an ML model (Random Forest, Gradient Boosting, etc.)
    4. Validate on held-out data
    5. Predict correction factors on a European grid
...
def load_cluster_corrections(corrections_csv):
    """Load correction factors from a cluster-based CSV.

    Args:
        corrections_csv: Path to the corrections CSV.

    Returns:
        DataFrame with columns: cluster, scalar, offset.
    """
...
def load_turbines_with_clusters(turbines_path, country_code):
    ...
    Args:
        turbines_path: Path to turbine CSV or GeoPackage.
        country_code: ISO country code to filter.

    Returns:
        DataFrame with lon, lat, cluster, and optional type columns, or None.
    """
...
def merge_corrections_with_locations(corrections_df, turbines_df):
    ...
    Args:
        corrections_df: Corrections with cluster IDs.
        turbines_df: Turbine locations with cluster IDs.

    Returns:
        DataFrame with lon, lat, scalar, offset, and optional type columns.
    """
...
def load_turbine_metadata(country_code):
    """Load turbine metadata (hub height, rotor diameter).

    Args:
        country_code: ISO country code for metadata lookup.

    Returns:
        DataFrame with turbine characteristics, or None if unavailable.
    """
...
def load_country_corrections(country_code, config):
    ...
    Args:
        ...
        config: Country configuration entry from `TRAINING_COUNTRIES`.

    Returns:
        DataFrame with lon, lat, scalar, offset, type, and country. Turbine
        metadata columns are included when available.
    """
...
def extract_spatial_context_features(corrections_df, radius_km=50):
    """Extract spatial context features from neighboring corrections.

    Args:
        corrections_df: Corrections DataFrame with lon/lat and scalar.
        radius_km: Neighborhood radius in kilometers.

    Returns:
        DataFrame with added spatial context features.
    """
...
def extract_era5_mismatch_features(corrections_df, terrain_nc, era5_base_dir='input/era5'):
    """Extract ERA5 terrain representation mismatch features.

    Args:
        corrections_df: Corrections DataFrame with lon/lat.
        terrain_nc: Path to high-resolution terrain NetCDF.
        era5_base_dir: Base directory for ERA5 files.

    Returns:
        DataFrame with added mismatch features.
    """
...
def extract_climate_features(corrections_df, era5_base_dir='input/era5', fast_mode=False):
    ...
    Args:
        corrections_df: Corrections DataFrame with lon/lat.
        era5_base_dir: Base directory for ERA5 files.
        fast_mode: If True, skip climate feature extraction.

    Returns:
        DataFrame with climate statistics for each point.
    """
    if fast_mode:
...
def download_land_cover(output_dir='input/landcover'):
    ...
    Args:
        output_dir: Directory for land cover data.

    Returns:
        Path to the land cover raster, or None if unavailable.
    """
...
def extract_land_cover(corrections_df, land_cover_raster=None):
    ...
    Args:
        corrections_df: Corrections DataFrame with lon/lat.
        land_cover_raster: Optional path to land cover raster.

    Returns:
        DataFrame with land cover and/or geographic features.
    """
...
def extract_terrain_features(corrections_df, terrain_nc, coastline_geojson=None, 
    ...
    """Extract terrain, climate, and land cover features at correction points.

    Args:
        corrections_df: Corrections DataFrame with lon/lat.
        terrain_nc: Path to terrain NetCDF.
        coastline_geojson: Optional coastline GeoJSON for distance-to-coast.
        era5_dir: Base directory for ERA5 data.
        land_cover_raster: Optional land cover raster path.
        fast_mode: If True, skip climate features.
...
def train_ml_model(X, y, model_type='random_forest', cv_folds=5, test_size=0.2, 
    ...
    """Train an ML model with cross-validation.

    If `X_validation` and `y_validation` are provided, evaluates on a spatial
    holdout set; otherwise uses a random train/test split.
    ...
    Args:
        ...
        test_size: Test fraction for random split.
        X_validation: Optional validation features (spatial holdout).
        y_validation: Optional validation targets (spatial holdout).

    Returns:
        Dictionary with trained model, scaler, and performance metrics.
    """
...
def plot_results(results, target_name='Scalar', output_dir='ml_europe'):
    """Create diagnostic plots for model predictions.

    Args:
        results: Results dictionary from `train_ml_model`.
        target_name: Target label for plot titles.
...
def predict_on_european_grid(model, scaler, feature_cols, terrain_nc, 
    ...
    """Predict correction factors on a regular European grid.
    ...
    Args:
        ...
        terrain_nc: Path to terrain NetCDF.
        coastline_geojson: Optional coastline GeoJSON for distance-to-coast.
        bounds: Optional bounds tuple (lat_min, lat_max, lon_min, lon_max).

    Returns:
        Dataset with correction predictions on the grid.
    """
...
def main():
    """Run the end-to-end ML training workflow."""
