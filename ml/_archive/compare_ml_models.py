...
This script trains and compares several algorithms to find the best model for
predicting correction factors from terrain features.
...
def get_model_configs():
    ...
    Returns:
        Dictionary keyed by model ID with configuration metadata.
    """
    configs = {
        'random_forest': {
            'name': 'Random Forest',
...
def train_and_evaluate_model(model, X_train, X_test, y_train, y_test, 
    ...
    Returns:
        Dictionary with metrics, timings, and predictions.
    """
    # Scale if needed
    if scaler:
...
def compare_models(X, y, model_configs, test_size=0.2, cv_folds=5):
    ...
    Returns:
        Dictionary of model results keyed by model ID.
    """
    print("\nSplitting data...")
...
def plot_model_comparison(results, target_name, output_dir):
    ...
    Args:
        results: Model results dictionary from `compare_models`.
        target_name: Target label for figure titles.
