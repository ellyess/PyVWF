    ...
    return pd.DataFrame(
        {
            "ID": ["A", "B"],
            "site_name": ["PE A", "PE B"],
            "region": ["PATAGONIA", "PAMPA"],
            "provincia": ["CHUBUT", "BUENOS AIRES"],
        }
    )
...
def test_build_metadata_exclude_allows_missing():
    join = pd.DataFrame({"ID": ["A"], "lon": [-67.0], "lat": [-45.0], "capacity_mw": [99.0]})
    ...
    pd.DataFrame(
        {
            "ID": ["PE A", "PE A"],
            "year": [2021, 2024],
            **{f"obs_{m}": [0.4, 0.45] for m in range(1, 13)},
        }
    ).to_csv(ar / "ar_obs.csv", index=False)
