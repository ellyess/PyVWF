...
def windstats_output() -> pd.DataFrame:
    ...
    for k, unit in enumerate(["W1", "W2 ", "W3"]):
        for i, (y, m) in enumerate(MONTHS):
            rows.append({"ID": unit, "Year": y, "Month": m,
                         "Output": 1.0e5 / 3.0 * (k + 1) + 7.3 * i})
...
def client_csv_output(tmp_path: Path, unit: str) -> pd.DataFrame:
    meta = pd.DataFrame({"ID": ["A", "B"], "lon": [9.0, 9.5], "lat": [56.0, 56.2],
                         "capacity": [3.0, 2.0 / 3.0] if unit == "mw" else [3000.0, 2000.0 / 3.0],
                         "height": [100.0, 90.0], "diameter": [112.0, 90.0]})
    gen = pd.DataFrame([{"ID": uid, "year": y, "month": m,
                         "energy": (1.0e3 / 7.0) * (j + 1) + 0.1 * i}
                        for j, uid in enumerate(["A", "B"]) for i, (y, m) in enumerate(MONTHS)])
    mp, gp = tmp_path / "meta.csv", tmp_path / "gen.csv"
    meta.to_csv(mp, index=False)
    gen.to_csv(gp, index=False)
    source = ClientCsvTurbineSource(mp, gp, country="DK", capacity_unit=unit,
                                    generation_column="energy")
