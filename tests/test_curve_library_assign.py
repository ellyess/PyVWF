...
MODELS = pd.DataFrame({
    ...
    "model": ["Vestas.V90.2000", "Gamesa.G90.2000", "Enercon.E82.2000",
              "Vestas.V80.2000", "2019COE_DW100_100kW_27.6"],
...
def fleet(model, diameter=90.0, capacity=2000.0, unit_id="a"):
    return pd.DataFrame({"ID": [unit_id], "model": [model],
                         "diameter": [diameter], "capacity": [capacity]})


def assignment(model, own, **kwargs):
    f = fleet(model, **kwargs)
    return assign.other_brand_assignment(
        f, pd.Series([own]), MODELS, rating_kw=f["capacity"])
...
def test_the_moved_share_is_by_capacity_not_by_count():
    f = pd.DataFrame({
        "ID": ["big", "small"], "model": ["Vestas.V90.2000", "Gamesa.G90.2000"],
        "diameter": [90.0, 90.0], "capacity": [9000.0, 1000.0],
    })
...
def test_specific_power_is_watts_per_swept_area():
    assert assign.specific_power(pd.Series([2000.0]), pd.Series([90.0])).iloc[0] == \
        pytest.approx(2000 * 1000 / (3.14159265 * 45 ** 2), rel=1e-6)
