...
def calls(monkeypatch):
    ...
    for name in ("run_train", "run_evaluate", "run_transfer"):
        monkeypatch.setattr(driver, name,
                            lambda *a, _n=name, **k: seen.append((_n, a, k)) or "run-dir")
    return seen


def test_validate_train(calls, capsys):
    assert validate.main(["train", "--region", "configs/regions/nz.toml"]) == 0
    assert calls == [("run_train", ("spec:nz.toml", "output/validation"),
                      {"mode": "all", "run_name": None})]
...
def test_validate_evaluate_and_transfer(calls):
    ...
    validate.main(["transfer", "--region", "uk.toml", "--source-region", "au.toml",
                   "--source-run", "s", "--mode", "onshore"])
    assert calls[0] == ("run_evaluate", ("spec:r.toml", "t", "output/validation"),
                        {"year": 2020, "mode": "all", "run_name": None})
    assert calls[1] == ("run_transfer", ("spec:au.toml", "s", "spec:uk.toml", "output/validation"),
