...
def test_es_metadata_column_mapping():
    md = pd.DataFrame(
        {
            "Unnamed: 0": ["A FARM 1", "A FARM 2"],
            "Manufacturer": ["Gamesa", "Made"],
            "kW": [660.0, 850.0],
            "Rotor (m)": [47.0, 52.0],
            "Tower (m)": [45.0, 55.0],
        }
...
def test_se_links_on_location_and_reads_onoffshore():
    md = pd.DataFrame(
        {
            "Unnamed: 0": [1, 2],
            "Location": ["Aapua", "Lillgrund"],
            "Manufacturer": ["Enercon", "Siemens"],
...
def test_monthly_cf_from_output():
    ...
    data = pd.DataFrame(
        {"ID": ["A"], "Year": [1999], "Month": [4], "Output": [660 * 0.3 * 30 * 24]}
    )
...
def test_twp_to_coords_fuzzy_matches_stripping_suffix():
    geo = pd.DataFrame({"ws": ["FARM X 1", "FARM Y 1"], "twp": ["Aapua", "Nonexistent Place"]})
    gwpt = pd.DataFrame(
        {"Project Name": ["Aapua wind farm"], "Longitude": [23.4], "Latitude": [66.5]}
    )
...
def test_source_resolves_and_loads(monkeypatch, tmp_path):
    ...
    pd.DataFrame(
        {"ID": ["a", "a"], "year": [1998, 2000], **{f"obs_{m}": [0.3, 0.35] for m in range(1, 13)}}
