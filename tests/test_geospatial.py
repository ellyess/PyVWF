...
def geojson(tmp_path, name, *polygons):
    ...
    path.write_text(json.dumps({
        ...
        "features": [{"type": "Feature", "properties": {"n": i}, "geometry": g}
...
def shapes(tmp_path):
    ...
    return (geojson(tmp_path, "onshore", square(0, 50, 2, 52)),
            geojson(tmp_path, "offshore", square(1, 50, 3, 52)))


def points(lons, index=None):
    return pd.DataFrame({"lon": list(lons), "lat": [51.0] * len(lons)},
...
def test_a_point_is_labelled_by_the_shapes_it_falls_in(shapes, method):
    on, off = shapes
    got = geospatial.add_domain_column(points([0.5, 2.5, 9.0]), onshore_geojson=on,
                                       offshore_geojson=off, method=method)
...
def test_a_point_in_both_domains_takes_the_preference(shapes, method):
    ...
    both = points([1.5])
    assert geospatial.add_domain_column(both.copy(), prefer_onshore=True, **kw)["domain"][0] \
        == "onshore"
    assert geospatial.add_domain_column(both.copy(), prefer_onshore=False, **kw)["domain"][0] \
        == "offshore"
...
def test_the_two_implementations_agree(shapes):
    ...
    a = geospatial.categorize_points_spatial_join(frame, onshore_geojson=on,
                                                  offshore_geojson=off)
    b = geospatial.categorize_points_by_region(frame, onshore_geojson=on,
                                               offshore_geojson=off)
...
def test_a_point_matching_two_polygons_of_one_file_stays_one_row(tmp_path):
    ...
    got = geospatial.categorize_points_spatial_join(points([1.5, 8.5]),
                                                    onshore_geojson=on, offshore_geojson=off)
...
def test_a_frame_with_repeated_index_labels_is_classified_positionally(shapes):
    ...
    got = geospatial.categorize_points_spatial_join(frame, onshore_geojson=on,
                                                    offshore_geojson=off)
...
def test_a_boundary_point_is_unknown_rather_than_either(shapes):
    ...
    on, off = shapes
    got = geospatial.categorize_points_spatial_join(points([0.0]), onshore_geojson=on,
                                                    offshore_geojson=off)
...
def test_an_empty_shape_file_is_refused(tmp_path, shapes):
    ...
    with pytest.raises(ValueError, match="No geometries"):
        geospatial.categorize_points_spatial_join(points([0.5]), onshore_geojson=empty,
                                                  offshore_geojson=off)
...
def test_an_existing_domain_column_is_not_overwritten_silently(shapes):
    ...
    got = geospatial.add_domain_column(frame, onshore_geojson=on, offshore_geojson=off,
                                       overwrite=True)
...
def test_an_unknown_method_or_domain_is_refused(shapes):
    ...
    with pytest.raises(ValueError, match="Unknown method"):
        geospatial.add_domain_column(points([0.5]), onshore_geojson=on,
                                     offshore_geojson=off, method="kriging")
