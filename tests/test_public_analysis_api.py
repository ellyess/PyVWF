...
def private_vwf_references(source: str) -> list[str]:
    """Every ``_name`` a module imports from vwf or reads off a vwf-bound name."""
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and (node.module or "").split(".")[0] == "vwf":
...
def test_the_detector_finds_what_it_should_and_nothing_else():
    bad = (
        "from vwf.harness.driver import _tidy_eval_frame, load_obs_and_fleet\n"
        "from vwf.harness import driver\n"
        "import vwf.correction\n"
        "import vwf.wind as w\n"
        "driver._SCOPE_KEYS\n"
        "vwf.correction._find_offset_iterative\n"
