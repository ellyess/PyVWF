...
def test_the_year_span_fixes_the_request_count():
    ...
    _, years = fetch.resolve_spec(args(code="eu", bbox=EU_BOX, years=list(range(2015, 2024))))
...
def test_a_worker_reports_a_refusal_instead_of_raising(tmp_path):
    ...
    result = fetch.fetch_chunk(
        lambda: client, spec, tmp_path, "eu", 2015, _chunk(tmp_path, 2015, [1])
    )
...
def test_a_worker_writes_its_months_and_clears_its_part_file(tmp_path):
    ...
    result = fetch.fetch_chunk(
        lambda: FakeClient(2015, [1, 2, 3]), spec, tmp_path, "eu", 2015, chunk
    )
...
def test_a_split_that_fails_part_way_leaves_what_it_wrote(tmp_path):
    ...
    result = fetch.fetch_chunk(
        lambda: ShortClient(2015, [1, 2, 3]), spec, tmp_path, "eu", 2015, chunk
    )
...
def test_one_refused_chunk_does_not_lose_the_others(tmp_path):
    ...
    clients = fetch.thread_local_clients(
        lambda: FakeClient(2015, list(range(1, 13)), refuse_months=[7, 8, 9], source=source)
    )

    with ThreadPoolExecutor(max_workers=2) as pool:
        futures = [
            pool.submit(fetch.fetch_chunk, clients, spec, tmp_path, "eu", y, c) for y, c in chunks
        ]
...
def test_the_split_step_never_runs_in_two_workers_at_once(tmp_path, monkeypatch):
    ...
    clients = fetch.thread_local_clients(lambda: FakeClient(2015, [1, 2, 3, 4], source=source))
    with ThreadPoolExecutor(max_workers=4) as pool:
        futures = [
            pool.submit(
                fetch.fetch_chunk, clients, spec, tmp_path, "eu", 2015, _chunk(tmp_path, 2015, ms)
            )
            for ms in groups
        ]
