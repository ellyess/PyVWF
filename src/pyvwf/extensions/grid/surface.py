...
Ported on 2026-09-13 from ``development:src/pyvwf/extensions/grid/atlite_export.py``.
...
:mod:`pyvwf.extensions.grid.interpolation`, which this calls rather than
reimplementing.
...
- kriging goes through :func:`pyvwf.extensions.grid.interpolation.kriging_at`, so
...
from pyvwf.geospatial import categorize_points_spatial_join, union_geometries
from pyvwf.harness.corrections import PLAUSIBLE_SCALAR
...
#: than correcting them. Same quantity as ``pyvwf.wind.fit_diagnostics``.
...
def area_mask(lon: np.ndarray, lat: np.ndarray, geojson_path, *, name: str) -> xr.DataArray:
    ...
    duplicate-row defect fixed in :mod:`pyvwf.geospatial`, and it is worth saying
