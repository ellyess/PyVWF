...
inherit the Northern-Hemisphere mapping hardcoded in :mod:`pyvwf.time_utils`
...
#: pyvwf.time_utils.add_time_resolution_columns, plus per-month).
...
def load_region(path: str | Path) -> RegionSpec:
    ...
    roughness = str(era5.get("roughness", "derived")).strip()
