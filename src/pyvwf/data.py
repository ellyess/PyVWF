...
Observation Sources (see pyvwf.sources):
...
Country-level observations and grid fleets are prepared in
``pyvwf.country_level``; their names are imported back here.
...
from pyvwf.sources import ObservationSource, resolve
from pyvwf.sources.base import ObsLevel

# The country-level helpers moved to pyvwf.country_level on 2026-09-25; the
# names are imported back because the harness, tests and scripts use them here.
from pyvwf.country_level import (
    OBS_SAME_TOLERANCE as OBS_SAME_TOLERANCE,
    _country_cluster_means,
    assign_country_clusters,
    country_cf_to_monthly,
    country_obs_is_per_cluster as country_obs_is_per_cluster,
...
def prep_country(
    ...
    *,
    ...
    Dispatches to an :class:`~pyvwf.sources.base.ObservationSource`. When ``source``
...
def cluster_train_set(
    ...
    while :func:`pyvwf.correction.find_offsets_country_level` and the harness's
    ...
    Args:
        ...
        min_cluster_size: Forwarded to :func:`pyvwf.clustering.cluster_turbines`;
...
def format_bc_factors(train_bias_df, time_res):
    ...
    **accepted years**, those whose offset was fitted and accepted. A year with
    ...
    - **Refused:** fits were attempted, but the accepted years are not a
      ...
      :func:`pyvwf.harness.corrections.fit_quality` counts it as a failed offset.
