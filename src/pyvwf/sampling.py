"""Sampling grids and their Voronoi cluster geometries, for country-level fleets.

A country-level region is simulated on a regular grid of points inside the
country's shape rather than on real turbines. ``create_sampling_points`` builds
that grid and ``add_turbine_metadata`` gives every point the representative
turbine; ``cluster_with_geometries`` clusters the points and returns the
clusters' Voronoi regions, clipped to the country, as the correction regions.
Split from ``pyvwf.clustering`` on 2026-09-25; it uses that module's country
shapes and its one-thread KMeans setting.
"""
...
from pyvwf.clustering import _ONE_THREAD, get_country_shape
