"""Country-level observations and grid fleets, for ``pyvwf.data``.
...
``pyvwf.data`` on 2026-09-25, which imports every name back.
"""
...
from pyvwf.curves import _default_power_curve
...
def country_obs_is_per_cluster(train_bias_df, time_res):
    ...
    :func:`pyvwf.correction.find_offsets_country_level` returns wherever L-BFGS-B
...
def _country_cluster_means(gen_cf, time_res):
    ...
    :func:`pyvwf.correction.calculate_scalar` applies at turbine level, so a
