...
def filter_to_bbox(metadata: pd.DataFrame, bbox: tuple[float, float, float, float]) -> pd.DataFrame:
    ...
    :func:`pyvwf.wind.aggregate_turbines_to_grid` assigns each turbine its
...
def assign_curves_from_library(metadata: pd.DataFrame, *, fallback_model: str) -> pd.DataFrame:
    ...
    This deliberately does NOT call :func:`pyvwf.curves.add_models`. That matches
...
def bin_hub_heights(metadata: pd.DataFrame, bin_m: float) -> pd.DataFrame:
    ...
    :func:`pyvwf.wind.interpolate_wind` builds a wind field with ONE HEIGHT LEVEL
    ...
    :func:`pyvwf.wind.aggregate_turbines_to_grid` already bins heights to 10 m
