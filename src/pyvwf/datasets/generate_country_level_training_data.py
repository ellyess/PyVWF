...
1. grid points with a representative turbine and their clusters
   (:mod:`pyvwf.datasets.country_grid`);
2. ENTSO-E observations for the training years and the test year
   (:mod:`pyvwf.datasets.entsoe_country_obs`).
...
Usage, from the repository root:
    ...
    ENTSOE_API_KEY=<key> PYTHONPATH=src python -m pyvwf.datasets.generate_country_level_training_data
    PYTHONPATH=src python -m pyvwf.datasets.generate_country_level_training_data \
...
from pyvwf.datasets.country_grid import COUNTRY_CONFIGS, generate_grid_points
from pyvwf.datasets.entsoe_country_obs import fetch_observations
from pyvwf.datasets.fetch_entsoe_capacity_factors import ENTSOEWindDataFetcher
