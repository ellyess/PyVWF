...
This adapter wraps the existing CSV loaders in :mod:`pyvwf.loaders.turbine_loaders`
and reproduces the metadata and capacity-factor preparation that
``pyvwf.data.prep_country`` performed inline before the source refactor.
"""
...
from pyvwf.curves import add_models
from pyvwf.time_utils import month_days

from pyvwf.loaders.turbine_loaders import load_turbine_metadata, load_turbine_observations
from pyvwf.sources.base import ObservationSource, ObsLevel
from pyvwf.sources.registry import register
...
class EuropeanTurbineSource(ObservationSource):
    ...
    through :mod:`pyvwf.loaders.turbine_loaders` and adapts them to the
    :class:`~pyvwf.sources.base.ObservationSource` contract. Resolved
    automatically by :func:`pyvwf.sources.registry.resolve` for its three
    country codes.
    ...
    def load_metadata(self) -> pd.DataFrame:
        ...
        :func:`pyvwf.curves.add_models` matches every turbine to a curve in the
