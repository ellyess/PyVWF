...
``pyvwf.datasets.generate_country_level_training_data``:
...
Like :class:`~pyvwf.sources.in_memory.InMemoryCountrySource` it is
...
from pyvwf.loaders.country_obs_checks import CountryObsReport, check_country_cf
...
class EntsoeFileSource(ObservationSource):
    ...
    def _obs_candidates(self) -> list[Path]:
        ...
        ``pyvwf.datasets.generate_country_level_training_data``. Both names are
    ...
    def load_observations(
        ...
        :mod:`pyvwf.loaders.country_obs_checks`. A failed gate warns rather than
