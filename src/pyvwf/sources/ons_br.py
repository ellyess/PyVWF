...
- **Curtailment screening is a first-class option.** The Nordeste is heavily
  ...
  ``pyvwf.datasets.ons_br.curtailment_mask_months``) sets the affected months to
...
from pyvwf.datasets.ons_br import DEFAULT_MIN_COVERAGE, monthly_cf_from_fc
...
def fc_to_monthly_cf(
    ...
    *,
    ...
    Thin finalisation over :func:`pyvwf.datasets.ons_br.monthly_cf_from_fc`: the
...
class ONSBrazilSource(ObservationSource):
    ...
    ``br_curtailment_mask.csv`` (optional)
        ...
        ``pyvwf.datasets.ons_br.curtailment_mask_months``.
    """
