...
from pyvwf.utils import ensure_numeric
...
def default_curve_key(power_curves: pd.DataFrame) -> str | None:
    ...
    curve (see ``_CurveByModel``), and ``pyvwf.curves._default_power_curve`` assigns
    ...
    on that library simulated with. ``pyvwf.provenance.curve_resolution``
...
def aggregate_turbines_to_grid(turb_info: pd.DataFrame, reanalysis) -> pd.DataFrame:
    ...
    # OPTIMIZATION: Use pyvwf.utils helper (2-3x faster)
