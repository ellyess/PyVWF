...
from pyvwf.wind import default_curve_key
...
def _bundled_hash(filename: str) -> str | None:
    """SHA-256 of the synthetic reference table bundled in pyvwf.resources."""
    packaged = Path(str(resources.files("pyvwf.resources") / filename))
...
#: ``model_match`` by :func:`pyvwf.curves.add_models` at load time.
...
def _bundled_curve_hashes() -> frozenset[str]:
    packaged = Path(str(resources.files("pyvwf.resources") / "power_curves.csv"))
    if not packaged.is_file():
        return frozenset()
...
def curve_resolution(fleet: pd.DataFrame, power_curves: pd.DataFrame) -> pd.DataFrame:
    ...
    key the table lacks, silently uses :func:`pyvwf.wind.default_curve_key`'s
