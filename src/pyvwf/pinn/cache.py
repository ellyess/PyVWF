...
``pyvwf.data.train_set`` and ``pyvwf.data.val_set`` use, so the model is fitted to
...
from pyvwf.data import clean_obs_data, country_cf_to_monthly, prep_country
from pyvwf.curves import load_power_curves
from pyvwf.harness.driver import resolve_source
...
from pyvwf.pinn.terrain import terrain_descriptors
...
def _country_observations(
    ...
    monthly capacity factor (``pyvwf.data.country_cf_to_monthly``), restricted
...
def prepare_grid(grid: pd.DataFrame) -> pd.DataFrame:
    ...
    ``pyvwf.data.prepare_country_fleet`` drops them. The conversion from MW puts
...
def _curve_matrix(
    ...
    for m in wanted:
        if m in available:
            ...
            missing.append(m)
    if missing:
        # Mirror pyvwf.wind's warned fallback rather than failing the whole region.
        print(
...
def era5_record_for(
    ...
    :func:`pyvwf.pinn.train.RegionTensors.from_cache` drops them.
    """
    ...
    return {
        ...
        "era5_extent": {
            **coverage,
            ...
            "meaning": (
                "units outside the loaded ERA5 extent are not simulated by pyvwf.pinn: "
