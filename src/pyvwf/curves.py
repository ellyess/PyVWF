...
Moved from ``pyvwf.data`` so that the adapters under ``pyvwf.sources``, which
assign curves at load time, can import it without importing the data
orchestration above them. ``pyvwf.data`` imports these names back for its own
use.
"""
...
def _default_power_curve(power_curves: pd.DataFrame) -> str:
    ...
    (:func:`pyvwf.wind.default_curve_key`), so the two cannot drift apart.
    """
...
def load_power_curves():
    ...
    :meth:`pyvwf.config.PyVWFPaths.reference_file`.
...
def add_models(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # got its curves (pyvwf.provenance.curve_resolution reads this).
